...
export default function NewUserDialog({
  ...
  useEffect(() => {
    ...
    let fdtCustomer : any = dbtSavCustomer?.filter((e: {customer_type_id: Number}) => Number(e.customer_type_id) === Number(customerType));
    setDdlCustomerTable(fdtCustomer);

    // setDdlCustomerTypeSelId(fdtCustomerType?.[0]?.id);

    if (selCustomerTypeId > 0) {
      fdtCustomer = fdtCustomer?.filter((item: { id: Number }) => Number(item.id) === selCustomerTypeId);
  ...
  }, [dbtSavCustomer, getValues, selCustomerTypeId, selectedUser?.customer_id, setValue]);
  ...
  }, [dbtSelCountry, selectedUser?.conuntry_id_code, setValue]);
  ...
  }, [dbtSelProvState, selCountryId, selectedUser?.prov_state_id_code, setValue]);
